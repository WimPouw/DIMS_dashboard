...
renderTrajectoryPlot() {
    ...
    displayRQAPlots() {
        ...
        const entries = Object.entries(this.rqaData.rqa_data);
        const single = entries.length === 1;

        // --- grid wrapper ---
        const grid = document.createElement('div');
        grid.style.display = 'grid';
        grid.style.gridTemplateColumns = single ? '1fr' : 'repeat(2, 1fr)'; // single plot = full width
        grid.style.gap = '20px';
        ...
        // Create plot tiles
        const plotConfigs = [];
        entries.forEach(([dataType, plotData], index) => {
            const plotDiv = document.createElement('div');
            // Keep id compatible with updateRQAHighlights()
            plotDiv.id = `rqa-plot-${index}`;

            // Bigger size when there’s only one plot
            plotDiv.style.height = single ? '80vh' : '60vh';
            plotDiv.style.minHeight = single ? '700px' : '500px';
        ...
        // Render after DOM paints
        setTimeout(() => {
            plotConfigs.forEach(cfg => {
                try {
                    // Clean up any previous plot instance (avoids stale layout)
                    try { Plotly.purge(cfg.containerId); } catch (e) {}
                    this.createRQAPlot(cfg.containerId, cfg.dataType, cfg.plotData);
                } catch (error) {
                    console.error(`Error creating RQA plot for ${cfg.dataType}:`, error);
                    const plotDiv = document.getElementById(cfg.containerId);
                    if (plotDiv) {
                        plotDiv.innerHTML = `<div style="color: red; padding: 20px;">Error creating plot: ${error.message}</div>`;
                    }
                }
            ...
            // If a time was already selected, immediately draw highlights
            if (this.lastClickedPoint !== null) {
                this.updateRQAHighlights();
            }

            // Keep plots responsive to container resizes
            try {
                const ro = new ResizeObserver(() => {
                    grid.querySelectorAll('.js-plotly-plot').forEach(el => {
                        try { Plotly.Plots.resize(el); } catch (e) {}
                    });
                });
                ro.observe(container);
            } catch (e) {
                // ResizeObserver not available; fallback on window resize
            ...
            this.showStatus(entries.length === 1
                ? 'RQA plot loaded. Click on the plot to select a time point.'
                : 'RQA plots loaded. Click on any plot to select a time point.');
        }, 50);
    }
    ...
    const container = document.getElementById(containerId);
    if (!container) throw new Error(`Container ${containerId} not found in DOM`);
    if (!plotData?.visualization) throw new Error('Missing visualization data');
    ...
    // ---- sort time & data together (avoid wrap issues) ----
    const pairs = vis.time.map((t, i) => ({ time: t, data: vis.data[i] }));
    pairs.sort((a, b) => a.time - b.time);
    const sortedTime = pairs.map(p => p.time);

    // ---- align RP time to start at 0 (handles JSON with t0>0) ----
    const t0 = (sortedTime.length ? sortedTime[0] : 0) || 0;
    const alignedTime = sortedTime.map(t => t - t0);
    if (t0 !== 0) console.log(`[RQA] Applying time offset t0=${t0.toFixed(3)}s so RP starts at 0`);

    // ---- derive global time window from loaded timeseries ----
    // (this is the range the rest of your app uses, e.g., your slider/video)
    let globalMax = -Infinity;
    let globalMin =  Infinity;
    if (Array.isArray(this.currentData)) {
        this.currentData.forEach(ds => {
            if (Array.isArray(ds?.data) && ds.data.length) {
    ...
    // fallbacks if no timeseries present
    if (!isFinite(globalMax)) globalMax = sortedTime[sortedTime.length - 1] ?? 0;
    if (!isFinite(globalMin)) globalMin = sortedTime[0] ?? 0;

    // convert the visible max into RP-local coordinates
    const visibleMaxLocal = Math.max(0, globalMax - t0);
    const visibleMinLocal = Math.max(0, globalMin - t0); // usually 0

    // For reference/debugging
    const rpMaxLocal = alignedTime.length ? alignedTime[alignedTime.length - 1] : 0;
    console.log(`[RQA] global range: [${globalMin.toFixed(2)}, ${globalMax.toFixed(2)}], ` +
                `RP local range: [0, ${rpMaxLocal.toFixed(2)}], ` +
                `visible local max: ${visibleMaxLocal.toFixed(2)}`);

    // ---- build dense matrix from sparse ----
    const N = vis.matrix_size;
    const matrix = Array.from({ length: N }, () => new Array(N).fill(0));
    vis.sparse_matrix.forEach(([r, c]) => {
        if (r >= 0 && r < N && c >= 0 && c < N) matrix[r][c] = 1;
    });

    // ---- traces: ONLY the RP heatmap ----
    const traces = [{
        x: alignedTime,
        y: alignedTime,
        z: matrix,
    ...
    const layout = {
        ...
        xaxis: {
            title: 'Time (s)',
            gridcolor: '#444',
            // 👇 clamp to your app's global time window
            range: [visibleMinLocal, visibleMaxLocal]
        },
        yaxis: {
            title: 'Time (s)',
            gridcolor: '#444',
    ...
    // ---- yellow highlight (clamped to visible range) ----
    if (this.lastClickedPoint !== null) {
        const clickedLocal = this.lastClickedPoint - t0;

        const minLocal = visibleMinLocal;                                   // clamp to visible start
        const maxLocal = Math.min(visibleMaxLocal, rpMaxLocal);             // clamp to visible & RP end
        const rawWindow = parseFloat(document.getElementById('windowSize')?.value) || 5;
        const fullSpan = Math.max(0, maxLocal - minLocal);
        const win = Math.max(0, Math.min(rawWindow, fullSpan));

        let startLocal = clickedLocal - win / 2;
        let endLocal   = clickedLocal + win / 2;

        if (startLocal < minLocal) {
            startLocal = minLocal;
            endLocal = Math.min(minLocal + win, maxLocal);
        } else if (endLocal > maxLocal) {
            endLocal = maxLocal;
            startLocal = Math.max(maxLocal - win, minLocal);
        }

        layout.shapes = [
            { type: 'line', x0: startLocal, x1: startLocal, y0: minLocal, y1: maxLocal, line: { color: 'yellow', width: 2 }, xref: 'x', yref: 'y' },
            { type: 'line', x0: endLocal,   x1: endLocal,   y0: minLocal, y1: maxLocal, line: { color: 'yellow', width: 2 }, xref: 'x', yref: 'y' },
            { type: 'line', x0: minLocal, x1: maxLocal, y0: startLocal, y1: startLocal, line: { color: 'yellow', width: 2 }, xref: 'x', yref: 'y' },
            { type: 'line', x0: minLocal, x1: maxLocal, y0: endLocal,   y1: endLocal,   line: { color: 'yellow', width: 2 }, xref: 'x', yref: 'y' },
            { type: 'rect', x0: startLocal, x1: endLocal, y0: startLocal, y1: endLocal, fillcolor: 'yellow', opacity: 0.12, line: { width: 0 }, xref: 'x', yref: 'y' }
        ];
    }

    // ---- render (purge first to avoid stale state) ----
    ...
    // ---- click handler: local RP time -> global slider/video time ----
    document.getElementById(containerId).on('plotly_click', (data) => {
        if (data.points && data.points.length > 0) {
            const pt = data.points[0];
            if (pt.xaxis?._id === 'x' && pt.yaxis?._id === 'y') {
                const clickedLocal = pt.x;
                const clickedGlobal = clickedLocal + t0;
                console.log(`RQA clicked local=${clickedLocal.toFixed(2)}s → global=${clickedGlobal.toFixed(2)}s`);
                this.handleTimeClick(clickedGlobal);
        ...
        setupControls() {
            try {
                ...
                if (windowSizeEl && this.config.defaultWindowSize) {
                 ...
                 // Populate perspective selector
                const pSel = document.getElementById('perspectiveSelect');
                if (pSel) {
                  const list = (this.config.perspectives && Array.isArray(this.config.perspectives))
                    ? this.config.perspectives : [];
        ...
        setupEventListeners() {
            ...
            // Re-render videos immediately with the same selected time/window (or start)
            const t = this.lastClickedPoint ?? (this.timeSlider ? parseFloat(this.timeSlider.min) : 0);
        ...
        plotTimeseries(datasets, selectedTime = null) {
            ...
            const layout = {
                title: {
                    text: `Velocity of the Ball Over Time for Video ${this.currentVideoID}`,
                    font: { color: 'white' }
                },
                paper_bgcolor: '#111',
                plot_bgcolor: '#222',
    ...
    handleTimeClick(time) {
        ...
        updateVideos(clickTime, windowSize) {
             // Build videoSrc with perspective if possible
              const tmpl = this.config.videoSrcTemplate || 'assets/videos/{videoID}_{persp}.mp4';
              const fallbackTmpl = this.config.fallbackVideoSrcTemplate || 'assets/videos/{videoID}.mp4';

              // If user picked a perspective, try it; otherwise "auto": try the first in config.perspectives
              let src;
              const tryBuild = (persp) => tmpl.replace('{videoID}', this.currentVideoID).replace('{persp}', persp);
              if (this.currentPerspective) {
                src = tryBuild(this.currentPerspective);
              } else if (Array.isArray(this.config.perspectives) && this.config.perspectives.length) {
                src = tryBuild(this.config.perspectives[0]);
              }

              // Fallback to bare video if needed
              const videoSrc = src || fallbackTmpl.replace('{videoID}', this.currentVideoID);
        ...
        async loadVideoData(videoID) {
            ...
            try {
                ...
                this.crossWaveletData = null; // Reset cross-wavelet data for new video
